    ...
    public class SharedSteps{
        ...
        public async Task WhenICreateTheFollowingEstates(Table table){
            foreach (TableRow tableRow in table.Rows){
                ...
                await Retry.For(async () => { await this.TestingContext.DockerHelper.CreateEstateSubscriptions(estateName).ConfigureAwait(false); }, retryFor:TimeSpan.FromMinutes(2), retryInterval:TimeSpan.FromSeconds(30));
        ...
        public async Task WhenICreateTheFollowingSecurityUsers(Table table){
            foreach (TableRow tableRow in table.Rows){
                ...
                if (tableRow.ContainsKey("RoleName")){
                    // Voucher Redemption
                    String roleName = SpecflowTableHelper.GetStringRowValue(tableRow, "RoleName");
                ...
                else if (tableRow.ContainsKey("EstateName") && tableRow.ContainsKey("MerchantName") == false){
